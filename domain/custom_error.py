...
class QueueNotFoundError(DomainError):
    def __init__(self, queue_id: Optional[Union[str, UUID]] = None, message: Optional[str] = None):
...
class ConsultationNotFoundError(DomainError):
    def __init__(self, consultation_id: Optional[Union[str, UUID]] = None, message: Optional[str] = None):
...
class ConcurrentUpdateError(DomainError):
    ...
    def __init__(self, entity_name: str = "ข้อมูล", entity_id: Optional[Union[str, UUID]] = None):
        if entity_id:
            ...
            self.message = f"มีคนอื่นอัปเดต{entity_name}ไปแล้วก่อนหน้านี้ กรุณารีเฟรชหน้าจอครับ"
