...
def test_should_queue_service_issue_first_queue_successfully(
    queue_repo, queue_service, patient, vital_signs, today_date
):
...
def test_should_queue_service_raise_error_duplication_queue(
    queue_repo, queue_service, patient, vital_signs, today_date
):
...
def test_should_start_consultation_successfully(
    queue_repo, queue_service, patient, vital_signs, today_date
):
    new_queue = queue_service.issue_new_queue(
        patient_id=patient.id,
    ...
    assert (
        queue_repo.get_by_queue_id(queue_id=new_queue.id).status
...
def test_queue_service_should_complete_visit_when_is_valid(
...
def test_queue_service_should_raise_error_when_complete_visit_status_witting(
    queue_service,
    diagnosis,
    new_queue,
):
...
def test_queue_service_should_cancel_visit_when_status_witting(
    new_queue, diagnosis, patient, queue_service, queue_repo
):
    assert new_queue.status == QueueStatus.WAITING
...
def test_queue_service_should_raise_error_when_cancel_visit_whit_status_complete(
