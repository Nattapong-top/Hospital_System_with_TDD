...
def test_doctor_starts_consultation_should_change_status_to_IN_PROGRESS(
    new_patient, queue_service, vital_signs, today_date
):
    ...
    db_queue = queue_service.get_active_queue_by_patient(
        updated_queue.patient_id, today_date
    )
...
def test_doctor_complete_visit_should_change_status_to_COMPLETE(
    new_patient, queue_service, diagnosis, today_date, vital_signs
):
    # --- 1. เตรียมของ (Arrange) ---
    new_queue = queue_service.issue_new_queue(new_patient.id, today_date, vital_signs)
...
def test_doctor_cancel_visit_should_change_status_to_CANCELLED(
    new_patient, queue_service, today_date, new_queue
):
    active_queue = queue_service.get_active_queue_by_patient(
        new_queue.patient_id, today_date
    )
...
def test_doctor_cancel_visit_should_change_status_to_CANCELLED_when_status_IN_PROGRESS(
    new_patient, queue_service, today_date, new_queue
):
    active_queue = queue_service.get_active_queue_by_patient(
        new_queue.patient_id, today_date
...
def test_doctor_cannot_cancel_visit_should_raise_error_when_status_COMPLETE(
    new_patient, queue_service, diagnosis, today_date, new_queue
):
    # --- 1. เตรียมของ (Arrange) ---
    # ดึงคิวที่เพิ่งสร้างตอนลงทะเบียนมาดู (มันควรจะเป็นสถานะ WAITING)
    active_queue = queue_service.get_active_queue_by_patient(
