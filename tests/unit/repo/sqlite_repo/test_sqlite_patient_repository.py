...
from domain.value_object import NationalID, Name, PhoneNumber


# --- Test Cases: ไล่บี้ตั้งแต่ปกติจนถึงเคสแปลกๆ ---
def test_sql_patient_repository_should_save_retrieve_patient_when_info_valid(
...
def test_sql_patient_repository_should_raise_error_when_duplicate_national_id(
    patient_repo, patient
):
...
def test_sql_patient_repository_should_return_none_when_patient_not_found(
    patient_repo, patient
):
...
def test_sql_patient_repository_handle_special_characters_and_long_strings(
    patient_repo, patient
):
...
def test_sql_patient_repository_should_save_retrieve_patient_with_version(
    patient_repo, patient
):
    patient_repo.save(patient=patient)
